...
8. Тесты были написаны в привычном мне стиле, хотя, вариантов написания этих тестов очень много.Я же, несмотря на пожелания в тестовов, юзал то,что использовал сам)Но в современном мире их уже почти не пишут,
потому что есть плагины, которые генерируют быстро и хорошо, но я писал по-старинке...
...
4. Запустить AccessingDataJpaApplication, он выведет проверку работы репозитория для сущности Product в форме логов, также можно сделать запрос на REST-контроллер вида 
"GET "http://localhost:8080/api/products/search?name=[часть слова, по которой хотите найти]&sortDirection=[desc или asc для сортировке по цене]""
