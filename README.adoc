...
1. Сразу стоит сказать, что разработка велась чисто в initial директории, как и предполагается, я не создавал другие, как бы делал в реальности.
2. Как я уже сказал, я не создавал другие директории, хотя, на последнем рабочем месте было деление на api и src, то есть, в api интерфейсы, документирование, версионирование API, hardcode url и другие полезные вещи,
а в src уже фунционал, но так как нового я ничего не создавал, то всё перечисленное выше отсутствует в коде. Если оно надо, то, скажите, пожалуйста,я с радостью добавлю.
3. Ветки создавались от main и туда же вливались, одна часть тестового - одна ветка feature. Почему не создавал другие ветки? Потому что смысла особого нет, так как тут чисто парочку классов и при этом 
на какой-то GitFlow и CI/CD даже намёков нет.
4. Также начиная с первого задания я внедрил AuditingEntity и BaseEntity, так как у нас это считалось best practice, они могут использоваться для инкапсуляции общих полей в одну сущность,а также для аудирования,
мягкого удаления и так далее.
5. Сразу же после клонирования подрубил Lombok, так как он явно облегчает жизнь, но при этом надо знать, какие проблемы могут возникат из-за них и ставить их правильно.
6. Репозиторий был реализован при помощи Spring Data JPA с последующим добавлением методов, которых не хватало.
7. Контроллер был написан самым банальным образом, чисто создавался на один эндпоинт) Конечно, там можно много чего улучшить...
8. Тесты были написаны в привычном мне стиле, хотя, вариантов написания этих тестов очень много. Я же, несмотря на пожелания в тестовом, юзал то, что использовал сам)Но в современном мире их уже почти не пишут сами, потому что есть плагины, которые генерируют быстро и хорошо, но я писал по-старинке...
...
2. Отдельно открыть initial.

3. При желании прописать свою БД, а не H2.

4. Запустить AccessingDataJpaApplication, он выведет проверку работы репозитория для сущности Product в форме логов, также можно сделать запрос на REST-контроллер,например, вида 
http://localhost:8080/api/products/search?name=dell&sortDirection=desc
(лучше такие документации в Swagger оставлять, конечно...)
